...
class HudView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let textPoint = CGPoint(x: center.x - round(textSize.width / 2), y: center.y - round(textSize.width / 2) + boxHeight / 4)
    ...
    func show(animated: Bool) {
        if animated {
            ...
            UIView.animate(withDuration: 0.3, delay: 0.3, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.5, options: [], animations: {
                    self.alpha = 1
